...
- ~~Facet aggregation runs inside the existing per-day fan-out (`storage/search.go:42`)~~ **Corrected 2026-09-02 during implementation:** that anchor is the legacy `Search()` path. The live path is the bounded `SearchPage`, which hands the handler at most one page — faceting a page would make counts change as the user pages. Phase 1 therefore runs an opt-in **bounded scan** (`storage.Facets`: same query/sources/range, newest rows first, `MaxFacetSampleRows = 20,000`, only aggregatable fields requested — never `_raw`) that streams rows through the same aggregator `AggregateFacets` uses, and reports `computed_over` plus `sampled: true` when the window exceeded the cap. The legacy sort path (non-timestamp sorts) still aggregates its in-memory window directly. The UI caption becomes "top values in results" with the sampled qualifier when set.
