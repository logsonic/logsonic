...
## 2026-09-22 — Import UI review: a batch with one stack-trace file silently collapsed every other file — **Closed (fixed in scope)**

**Source:** review of the import redesign (`redesign/import-ui`) in the browser, light + dark, 1440 / 1280 / 900 px, with the four bundled samples `linux-syslog.log`, `nginx-access.log`, `app-json.log`, `java-stacktrace.log`.

**Severity:** P1 (silent data loss on the default path), plus three P3 UX gaps. All fixed on the branch; listed so the supervisor can see what the redesign's own verification walk missed and why.

**What was wrong:**

1. **Silent collapse of the batch.** Detection of `java-stacktrace.log` auto-applied its ISO8601 multiline header to the store's *session-wide* triple; `useUpload` then sent that header on every file's `/ingest/start`. Lines that never match an ISO8601 header are all continuation lines, so the syslog file folded into one record per 100 physical lines (log2grok's `MaxLines` default). Reproduced: 4 files → **43 events** (`linux-syslog` 1999 → 20 rows, `nginx-access` 500 → 5, `app-json` 500 → 5). The redesign made it invisible: `useFileDetection` deliberately skipped the wizard's re-detect after an auto-apply ("that batch already used the new config" — true only for the file that triggered it), so the other three rows kept their 100 % verdicts. The redesign's verification walk used files that trigger no multiline detection, so the path was never exercised.
   **Fix:** multiline is per file. `FileSessionOptions.multiline` (`FileMultiline {enabled, mode, headerPattern}`) is filled by detection for the file the suggester found it on, edited in that file's Options tab (which re-parses the file's preview under the new folding and drops the cached alternative scores), checked per file by the import gate, and sent per file by `useUpload`. The Options tab's batch link now copies only the smart decoder and timezone ("Apply decoder & timezone to all files") — stamping one file's header onto the rest is exactly the collapse. The store's global `sessionOptionsMultiline*` triple is now legacy (only `handlePatternOperation` reads it). Rows carry a wrap icon when a file is folded.
   **Backend:** `POST /parse` auto-detected a layout whenever no *enabled* config was sent, so "Off" could not be expressed to the preview — it re-folded on its own while `/ingest/start` (which never auto-detects) would not. `parse.go` now auto-detects only when the field is **absent**; an explicit `{"enabled": false}` means "do not fold", the same thing it means on ingest. `POST /timestamp/preview` gained the same optional `multiline` field (it decoded raw lines, so a timestamp override on a folded file mapped 20 unfolded verdicts onto 13 folded preview rows); the Timestamp tab sends the file's folding. `types.go` comment + swagger regenerated, `api-types.ts` mirror updated; `TestHandleParse_ExplicitMultilineOffSkipsAutodetect`, `TestHandleTimestampPreview_FoldsWithMultiline`.
   **Verified:** same 4-file drop → per-source rows **2000 / 500 / 500 / 13**; the java file's Options tab: Off → 50 preview rows at 26 % match, ISO8601 → 13 records at 100 %.

2. **A drop that missed a target navigated the tab away.** Once files are staged the only drop targets were the file-list rail; dropping on the preview pane (the largest surface) let the browser open the file and the staged session was gone. `Import.tsx` now owns window-level `dragover` / `drop` listeners: the page accepts drops anywhere while files can be staged, and swallows them otherwise. The targets' own handlers stop propagation, so nothing is added twice (verified with a synthetic `DataTransfer` drop on the preview pane and on the list).

3. **Reload / bookmark of `#/import` landed on Home.** `useSearchQueryParams`'s module-level init pushed `#?isRelative=…` over any hash without query params, including `#/import` and `#/settings/*`. It now pushes only on the landing route. Pre-existing, but the redesign's ISSUES note called it cosmetic.

4. **File size and line count never showed in the rail at laptop widths.** The container query hid `.ls-imp-rail-aux` at ≤ 280 px, and the rail's content box is ~260 px at 1440. The rail is now `clamp(260px, 22%, 400px)`; the size stays visible down to a 230 px container and only the `~N lines` estimate yields (at ≤ 360 px). Nothing truncates at 1440.

**Not changed (noted for the supervisor):**

- The spec's per-row "Detection failed — retry" affordance is still the header's "Re-detect all"; the detail panel's Pattern tab shows the error.
- The Timestamp tab's Year tile can read "2025 · inferred from mtime" while the picker's selected option reads "2026 · file mtime": both are right (the anchor year is 2026, the resolver rolled it back because Jun 14 is after the April mtime), but the two numbers side by side are confusing. Needs a wording decision.
- `saveDialogShown` is one-shot per page visit: cancel the import after skipping the dialog, then import again, and the SavePatternDialog does not re-offer.

**Verified by:** vitest 311/311 (new: `multilinePresets.test.ts`, `importGate.test.ts`, per-file + `reparseFile` cases in `useFileDetection.test.tsx`); `npx tsc` clean; eslint zero on every redesign / new file, legacy files' counts unchanged (`useUpload.ts` 177, `useImportStore.ts` 77, `useSearchQueryParams.ts` 112); `go vet` + `go test ./...`; `check-api-types.mjs` 173/173; swagger regenerated with the pinned swag; `e2e-test.mjs` 30/30 against the dev servers (the exact CI gate on the embedded binary was **not** run); `npm run build`; browser walk as above at 1440 / 1280 / 900, light + dark.
...
3. ~~**Multiline folding stays a session-wide setting**, shown in every file's Options tab. `useUpload.ts` sends the store's global triple on each ingest session and the spec forbids changing `useUpload`; making it per-file is a separate contract change.~~ **Overturned 2026-09-22** — see the review entry above: the session-wide setting silently collapsed every other file in a batch. Multiline is now per file (`ImportFile.sessionOptions.multiline`).
...
- ~~Pre-existing, observed while testing: `useSearchQueryParams` rewrites the URL hash to `#?since=…` on a cold load of `#/import` (the page still renders; HashRouter keeps the route). Cosmetic, not introduced here.~~ Not cosmetic — a reload or bookmark then landed on Home. Fixed 2026-09-22 (review entry above).
