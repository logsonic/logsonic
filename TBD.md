...
| 2026-09-02 | [now-11](specs/now-11-ci-quality-gate.md) | 🟡 Partial (phase 2b-ii/2) | **Phase 1 (2026-09-02):** `.github/workflows/ci.yml` (go / web / swift / snapshot / e2e jobs), `docs.yml` + `check-links.sh`, `run-e2e.sh`, and `frontend/e2e-network-audit.mjs` (the automated proof of principle D2). Commit `0687d74` (corrected here — this row previously cited `9aa91c9`, which does not exist in this repo's history; found while amending this row for phase 2a); preceded by `13167ea` clearing the staticcheck baseline (incl. removing the deprecated spoofable `RealIP` middleware — safe because `grep -rn RemoteAddr backend/` finds no reader outside chi's request logger). Verified by: YAML parsed; **every job's commands run locally** — staticcheck/race/swag-drift/govulncheck green, vitest + coverage (48.25 % statements baseline), `test-macos-app.sh` green, `goreleaser build --snapshot --id logsonic` green, `run-e2e.sh` green (audit: 34 requests, one origin, zero off-box; smoke 30/30), `check-links.sh` 101/101. Deliberate non-blocking gates: `tsc` (415 pre-existing errors) and eslint-on-changed-files (6,272 pre-existing) report counts to the job summary. RELEASE.md §1 was also updated in `0687d74` (line 13 now describes CI in full) — unrecorded at the time; noted here so phase 2a's acceptance-box check isn't mistaken for new work. **Phase 2a (2026-09-02):** `api-types.ts` mirror check (`.github/scripts/check-api-types.mjs` + `check-api-types.test.mjs`, warn-only, wired into `ci.yml` as its own job) and the lychee external-link check (`.github/lychee.toml` + a new `external-links` job in `docs.yml`, lychee 0.24.2 pinned and downloaded as a release binary). Also closed now-09's open S7 box: `e2e-network-audit.mjs` now asserts zero `securitypolicyviolation` events via `page.addInitScript` (present before the page's own scripts run, so load-time CSP blocks count too, not just interaction-time ones) — the pre-existing `page.on('request')` audit can't see a request the CSP blocked before it reached the network layer. Commit `e54a868`; preceded by two separate safe-fix commits found while building the checks — `347136a` (5 pre-existing Go↔TS field-mirror gaps: `ParseRequest.multi`, `AutosuggestResult.{coverage,timestamp_field,timestamp_layout,timestamp_source}`) and `ffd4643` (dead link `signpath.org/foundation` → `signpath.org` in `RELEASE.md`, found by lychee). Verified by: `node --test check-api-types.test.mjs` 5/5; `node check-api-types.mjs` 129/129 fields present; `lychee` 152 OK / 0 errors / 5 excluded against the same file set `check-links.sh` uses; `check-links.sh` 108/0 (unaffected); vitest 191/191 unchanged; tsc 415 unchanged baseline; eslint zero errors on every added line in both touched frontend files (verified by re-running eslint correctly rooted in `frontend/` — an earlier run from the wrong cwd had shown spurious no-undef errors on lines using `document`/`window`, which was a mistake in how the check was invoked, not a real regression); `actionlint` clean on `docs.yml` (`ci.yml`'s one hit is pre-existing, a line not touched here); embedded build + live server + `e2e-network-audit.mjs`: 34 requests, one origin, zero CSP violations, **plus a negative control** (a throwaway script forcing `fetch('https://example.com/')` against the same live server produced exactly one `connect-src` violation event, proving the new listener actually fires rather than being a silent no-op). **Workflows still unexecuted** — nothing is pushed; first run needs a push or PR (ISSUES.md). The "lint-delta gate" item from an earlier draft of this row is dropped — it was never in the now-11 spec. **Test table (phase 2a):** Q4 verified directly — a scratch copy of `types.go` with an added `zz_probe` field produced the `::warning::` line (with `GITHUB_ACTIONS=1`) and the plain form (without), exit 0 both times, confirming the warn branch isn't dead code. Q6 external half: lychee's failure path was seen for real before the two fixes (the 404s this session fixed), and is 0-error/exit-0 after; the relative-link half of Q6 is phase 1. Q2 CSP half: covered by the negative control described above; the request-audit's own non-loopback-request exit path (`page.on('request')`) was not re-exercised this session — that half's phase-1 verification stands. Q1/Q3/Q7 remain phase-1 claims pending an actual CI run (nothing pushed). Q5 is phase 2b-ii (bench harness not built). **Phase 2b-i (2026-09-02):** the doc-command executor (`.github/scripts/check-dev-commands.sh` + a `dev-commands` job in `docs.yml`). Runs, literally, the four `go run .` / `npm` lines in `docs/development.md` a first-time contributor hits before any test runner (`npm ci`, `go run . -port 8080 -auto-port=false`, `PORT=8081 npm run dev`, `npm run test`), each grepped verbatim out of the doc first so a doc edit that changes the command fails loudly instead of the script silently testing stale text. Commit `aa1d8f5`; preceded by a separate fix (`f4edfb7`) for a real bug found while building it — `go run .` (and `./scripts/dev.sh`) doesn't compile on a fresh clone because `backend/pkg/static/dist` is gitignored and `//go:embed all:dist` has nothing to embed until the frontend is built once, which `docs/development.md` never said; reproduced by hand (moved `dist/` aside, `go build ./...` failed with exactly that error, restored), fixed with a new "First-time setup" section. Servers are killed by port (`lsof -ti:<port> | xargs kill`, escalating to `-9`), not by the launching shell's PID, after a real bug was caught mid-development: `go run`/`npm run dev` each fork the process that actually listens, so killing the wrapper PID left the compiled binary/vite alive and still bound to the port (`ps` showed both alive well after the script believed the port was free). Verified by: two clean end-to-end runs (exit 0, `lsof`/`ps` empty after); a runtime-failure path (port 8080 pre-bound externally) — exactly 1 failure, non-zero exit, no orphans; a doc-drift path actually exercised (not just reasoned about) — a scratch copy of the doc with the backend command's port changed 8080→9999, script pointed at the copy: first attempt double-counted the failure (`check_doc_says` both counted and returned non-zero, so the caller's `else` branch counted it again) — caught in review before committing, fixed by making the check a pure predicate; re-run after the fix gave exactly 1 failure with the correct message and the independent steps still ran and passed. `POLL_TIMEOUT_BACKEND=180`: sized from a measured 41s cold `go build .` (empty module/build caches) on this machine, doubled for runner variance and rounded up — verified warm-local only; the real number on a GitHub-hosted runner is unverified until the first push. shellcheck and actionlint both clean; `xargs -r` confirmed safe on BSD xargs, not just GNU. **Phase 2b-ii (2026-09-02):** the bench harness (`backend/bench/` — deterministic corpus generator + import-throughput / term-search (common + rare selectivity) / term+facets / histogram / storage-open scenarios → `bench.json`), `.github/scripts/compare-bench.mjs` + its `node:test`, and `nightly.yml` (schedule + manual dispatch; also runs the existing `BenchmarkIndexSize*` Go benchmark and the full E2E set). Commits `a0e1b35` (a facets.go fix found while building the harness, see below and ISSUES.md) + `7bdb88e` (the harness itself). Verified locally at this machine's default scale — **100,000 lines, 365 real tiny day-indices (50 docs/day, since `storage_open_ms` measures per-index open overhead, not row count), 30 iterations + 5 warmup discarded** — not at the real §8 scale (10M lines): `import_throughput` 21,324 lines/sec (measures `Store()` only, no Grok parse — an *upper bound* on real-path import against §8's 100k lines/sec target, not the real-path number); `search_term_common` (`status:500`, 20,000 hits) p50 25.4 ms / p95 26.6 ms; `search_term_rare` (a unique `traceid`, 1 hit) p50 0.22 ms / p95 0.43 ms; `search_term_facets` p50 722 ms / p95 738 ms — **already over §8's 600 ms budget at 100k rows**, and since the scan is capped at `MaxFacetSampleRows` (20,000 rows), it will not improve at 10M rows — whether per-batch cost also grows with total index size (each batch is a Bleve query against the whole index, so postings intersection and top-K sort plausibly scale with it) is unmeasured until nightly runs at full scale (a now-02 phase-3 finding surfacing early, see ISSUES.md and the now-02 row); `histogram` (100 buckets, match-all) p50 163 ms / p95 169 ms; `storage_open_ms` (365 indices) 666 ms (the pre-next-10 eager-open baseline). Numbers are in this row and the `7bdb88e` commit message — there is no PR (nothing pushed) for the acceptance box's "PR description". **10M-line/full-scale numbers are unverified** — nightly.yml has never executed (nothing pushed); Q5 is correspondingly partial: the harness runs and produces `bench.json` with six of the eight §8 scenarios populated at dev scale (`sorted_field` and `idle_rss` deferred, below), but "on `main`" behavior is unproven. **Bug found and fixed while building this (own commit, `a0e1b35`):** `Storage.Facets()` could spin forever under an uncancelled context (bounded to ~60s over real HTTP requests by the API timeout middleware, so P1 rather than P0 — see ISSUES.md for the full severity analysis) when a tied sort key landed exactly on the shrinking tail window near `MaxFacetSampleRows`; confirmed reproducible, not theoretical. See ISSUES.md for detail and the amended search-after-seam entry. **Deferred, not silently dropped:** the `sorted_field` scenario (§8 itself assigns that budget to next-10, not this harness — `SearchPage` rejects `sort_by != "timestamp"`), `idle_rss` (§8 assigns this to an E2E check against `/info` + `ps`), a committed `backend/bench/baseline.json` (no real full-scale run has ever happened, so there is nothing honest to compare against — `compare-bench.mjs` is wired into `nightly.yml` conditionally on that file existing), corpus-artifact caching (would make `import_throughput` measure nothing). **Tracking correction (2026-09-03, found at now-08 pickup — same class as the `9aa91c9` correction on this row):** the line above listed step 6's README badge and `docs/development.md` CI section together as deferred here. The badge half was simply wrong: `README.md` already has the `ci.yml` badge, shipped in phase 1 (`git log -S"badge.svg" --oneline -- README.md` shows only `0687d74`, 2026-09-02) — not deferred, not this row's work. The CI-section half is more precise than "deferred" suggested: `docs/development.md` already has a "Continuous integration" section describing the `go`/`web`/`swift`/`snapshot` jobs (also `0687d74`), but — checked directly (`grep -n "nightly\|bench" docs/development.md` — no hits) — it does not mention `nightly.yml` or the bench harness added later in phase 2b-ii. So the residual gap and 2b-ii's stated reason for leaving it (`check-dev-commands.sh` greps specific doc lines verbatim, and a `go run ./bench` line risked a false match) both stand; only the badge claim was mistaken. |
| 2026-09-02 | [now-08](specs/now-08-native-path-import.md) | 🟡 Partial (phase 2/~4) | **Phase 1 (2026-09-02):** Backend-only: `POST /api/v1/ingest/file` reads a file by absolute path (gzip/zstd sniffed by magic bytes, rotated sets oldest-first via `include_rotated`) and stores it through the same `ingestBatch` pipeline the chunk-upload route uses, so the two routes are byte-identical (parity-tested). Synchronous in this phase — 200 with totals when done — registered outside the API timeout group next to `/live/stdin` so a large file doesn't 504; the spec's async job + SSE progress model is phase 2. `Config.WorkDir` (dead) deleted per spec. New `pkg/ingestfile` package has no HTTP imports, as required, for `now-04`/`now-12` to reuse. Commit `3ce78bc`. **Found uncommitted in the tree at pickup** — the implementation (reader package, handler, tests, the spec's own phase-split note) was already present with no TBD/ISSUES row and no commit; authorship isn't verifiable from the repo. Reviewed line-by-line, verified, and completed rather than discarded — see the commit message for the full disclosure. Two bugs found and fixed during that review: `api-client.ts`'s `ingestFile()` posted to a nonexistent `/ingestFile` route typed as the wrong response (live tsc error, confirmed by baseline diff: 416 vs. 415); and the new route sat outside the now-09 JSON-only CSRF middleware, reachable via a cross-origin `text/plain` form post with no preflight — fixed by applying `requireJSONBody` to the route directly and reordering session validation before the filesystem is touched (closes a secondary path-existence oracle). Verified by: `go build/vet/test/test -race` all green from `backend/`; `gofmt -l` clean; `go mod tidy` no-op; `swag init` re-diffed against committed docs, byte-identical; `npx vitest run` 191/191 unchanged; `npx eslint` on touched files, zero errors on touched lines (30 pre-existing baseline elsewhere); `npx tsc -b` back to the 415-error baseline. Test-table coverage: R1–R9 (`pkg/ingestfile`) and H2/H4 (handler parity + rotation/errors) all green. **Deferred at phase 1 (per spec's own phase split):** H1/H3/H5/H6/H7/H8 (job registry, SSE `ingest_progress`, cancellation, memory-at-scale, server-shutdown, Windows-path CI, `POST /parse/preview-file`), the macOS shell path handoff (`deliverNativePaths`, dropping the 512 MB cap and extension guard), wizard UI wiring to call the new endpoint, and the docs updates in spec step 6 (`docs/getting-started.md`, `docs/configuration.md`, `docs/live-streaming.md`) — none currently mention this endpoint, so nothing is stale. Native app drops are unaffected; `logsonicfile://` still handles them exactly as before. **Phase 2 (2026-09-03):** the async job model from the spec's "Asynchronous with progress (phase 2)" decision — `POST /ingest/file` now returns `202 {job_id}` immediately (moved from outside the API timeout group to inside it, since it returns in milliseconds like every other mutating route), the read runs on its own goroutine, `GET /ingest/jobs` lists jobs (running + a 1h-retention window after completion, swept by a new `StartIngestJobCleanup` on the same 5-minute cadence as session cleanup), `DELETE /ingest/jobs/{id}` cancels, and `ingest_progress` (state `running|done|cancelled|error`) broadcasts on the existing live SSE hub via a new `publishBroadcast` that reaches every subscriber regardless of `sourceFilter` or pause state — a paused tail feed must still learn a job finished, since `GET /ingest/jobs` is the only other way it would find out (design intent; the pause-bypass itself isn't exercised by a test — every SSE test in this session uses an unpaused subscriber). A job's own context is created before its goroutine starts (not inside it), closing a window where a DELETE landing immediately after the 202 could race an unset cancel func — found and fixed in this session's own remediation pass, before commit, so it never shipped. `context.Canceled` (DELETE, or the parent shutdown context) maps to state `cancelled`; `context.DeadlineExceeded` (a 6h wall-clock ceiling) maps to `error`. Progress counters update every ~1,000 physical lines, decoupled from the 10k-line store-batch size, so a slow multi-GB gzip member shows movement without waiting a full batch; broadcasts are rate-limited to one per 250 ms per spec. `rate_lines_per_s` freezes at `finishedAt` rather than continuing to decay against wall-clock time the longer a finished job sits unread in the registry. Consumer sweep (web client, native fetch, SSE consumer, CLI, MCP, macOS shell, demo/E2E scripts, container image, docs) found no live UI consumer of `ingestFile()` yet, so the 200→202 response-shape change breaks nothing; `api-types.ts`/`api-client.ts` mirror the new types and add `listIngestJobs`/`cancelIngestJob` regardless. Commit `bfafeaa`. Verified by: `go build/vet/test -race ./...` green from `backend/` (new tests: `TestIngestFileCancelViaDelete`, `TestIngestFileCancelledByServerShutdownContext`, `TestIngestFilePublishesProgressOverSSE`, `TestCancelUnknownIngestJobReturns404`, `TestPruneFinishedIngestJobsDropsOnlyOldTerminalJobs`, `TestIngestJobSnapshotFreezesRateAfterFinish`; the phase-1 parity/rotation tests rewritten to poll to a terminal state before ending the session); `gofmt` clean; `go mod tidy` no-op; `swag init` re-diffed clean; `npx vitest run` 191/191 unchanged; `npx eslint` on both touched frontend files at their pre-existing baseline (30/30, zero on added lines); `npx tsc -b` at the 415-error baseline; `node .github/scripts/check-api-types.mjs` 136/136 (up from 129). **Test-table coverage:** H2/H4 rewritten and green under the async model; H3 green. H1 partially verified — the SSE half (an `ingest_progress` event reaches a live subscriber and reports `state: done` for the right `job_id`) and poll-to-terminal are both exercised, but not at the spec's literal "10k-line fixture, `rows_stored == 10000`" scale, and `_seq` monotonicity is only implied by the parity test's row-order equality with the chunk path, not asserted directly. H6 is an **analog only**: a pre-cancelled parent context stands in for a real `Start()`/signal/`Shutdown()` sequence, not exercised end-to-end (see ISSUES.md — the same not-awaited-before-`CloseStorage` gap `TailManager` already has). H5 (memory at scale), H7 (Windows paths), H8 (`preview-file`) untouched by this phase, deferred per the spec's own step split. **Still deferred:** macOS shell path handoff (step 5), wizard UI wiring for progress/cancel (step 4), spec-step-6 docs including `docs/live-streaming.md` (see ISSUES.md — that doc doesn't document any SSE event schema today, existing or new, so this is deferred alongside the UI that will consume the event, not skipped). Two design choices and the shutdown-drain gap are in ISSUES.md rather than resolved here. |
| 2026-09-02 | [now-02](specs/now-02-faceted-fields-sidebar.md) | 🟡 Partial | **Tracking correction (2026-09-02, found at pickup — same class as the `9aa91c9` correction on the now-11 row, not a new package):** the "phase 3" framing in this row's previous revision doesn't match the spec (`specs/now-02-faceted-fields-sidebar.md` has no phase 3). Its three items dispose as follows, not as one open phase: (1) **MCP exposure of facets** — the spec's own "Out of scope" section sends this to `next-05-mcp-depth.md` explicitly; it was never now-02's work and is removed from "remaining" here. (2) **`_src` counts from the sources catalog** — genuinely blocked on `now-10` (unstarted); stays open. (3) **Performance manual-validation (spec step "Performance", "~20% budget")** — resolved by inspection, not by an HTTP-level rig (an HTTP measurement would add nothing: `logs.go:261` calls `SearchPage` then `Facets()` serially, so the handler-level number is exactly `search_term_common`'s 25 ms plus `search_term_facets`'s 722 ms already measured by now-11's harness — see that row). Traced where "search latency shown in the status bar" (the spec's literal wording) actually comes from: `StatusBar.tsx`'s displayed `Nms` reads `apiExecutionTime` from `useSearchQueryParamsStore`, which is set **only** by `setPerformanceMetrics()` in `useSearchLogs.ts`'s primary `fetchLogs()` call (line ~160) — a request that does not pass `include_facets` at all. The facets scan runs through the separately deferred `refreshSearchMetadata()` (added in phase 2 specifically to keep it off the primary request), which updates `total_count`/`log_distribution`/`facets` but never calls `setPerformanceMetrics`. **The status bar's number is therefore unaffected by `include_facets` regardless of scan cost — the spec's literal acceptance criterion passes by architecture**, not because the facets scan is fast. The 722 ms scan cost is real and is a Fields-panel-open cost (users see the panel populate ~700ms after results, not a slower search), not a "search latency" regression as the spec defines it — a product/UX question worth a separate look, not a spec-compliance failure. Also worth recording: the spec's own prescribed remedy if the budget were exceeded ("aggregate in the fan-out goroutines, step 2 alternative") targets the legacy `storage.Search()` path, which line 21's own in-spec correction already established is not the live path (`SearchPage`/`Facets` are) — so that remedy is dead advice, not an available fallback; the real lever is `next-10`'s lease-per-batch/cursor work, already tracked. See ISSUES.md for the full trace. **Still open:** the "#10 referenced in PR description" acceptance box — an outward-facing action tied to a PR, same class as now-07 task 4; no PR exists on this local-only, never-pushed workflow. **Phase 2 (2026-09-02):** Fields panel shipped — third sidebar tab + rail entry, top-5/8 values with counts and bars, click → `+field:"value"`, ⌥-click → `-field:"value"`, second click removes, polarity switch replaces the twin; caption states coverage and sampling; unqueryable field names render non-clickable. `lib/query-clauses.ts` (14 unit tests), `useFacetStore`, `refreshSearchMetadata()` extracted from `useSearchLogs` so facets ride the deferred request only while the panel is open. Verified by: vitest 191/191; new self-seeding `e2e-facets.mjs` 10/10 on the dev split and via `run-e2e.sh` against a fresh embedded build (now part of the CI e2e job); smoke 30/30; tsc unchanged; lint delta zero except one added import in `SidebarPanel.tsx`'s pre-rule import block. **Phase 1:** Backend: `include_facets=true` on `GET /api/v1/logs` returns the field/value summary (`computed_over`, `sampled`, per-field distinct + top-8 values, caps per spec) via a bounded 20k-row scan in `storage.Facets` — a recorded deviation from the spec's fan-out assumption, since the live `SearchPage` path only yields one page. DTOs in `pkg/types` + `api-types.ts`, Swagger regenerated, `mcp/SKILLS.md` documents the param. Commit `55975d1`. Verified by: storage unit tests B1–B8 + small-window + multi-batch exactness; HTTP tests H1/H2 (off path byte-equal modulo timings, no `facets` key)/H3; `go test -race`; BGL sample timing 5 ms → 13 ms — but that sample is 2k rows, not large (the spec and `specs/README.md` mislabelled it), so the spec's ≤20 % latency budget is untested at scale; phase 2 measures at 100k+ rows. Known trade-off: the scan holds the storage read lock for its whole duration (commented in `facets.go`; lease-per-batch queued for `next-10`). **Found and logged:** `SearchPage` duplicates rows at search-after seams when ≥50 rows share a timestamp (reproducer skipped in `search_page_seams_test.go`, ISSUES.md). Phase 2 not started: `useFacetStore`, `FieldsPanel.tsx`, rail entry, the Bleve value-escaping helper + tests, E2E `e2e-facets.mjs`, `facets_only` for MCP. |
