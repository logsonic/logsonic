...
- Build: GoReleaser produces stripped Linux `tar.gz` and Windows `zip` archives for amd64/arm64. macOS is arm64 (Apple Silicon) only for now — the installed Swift toolchain dropped x86_64 compatibility libs, so the previous `lipo`-merged universal darwin binary can't be built; no bare macOS archive is published either way. See [`backend/.goreleaser.yaml`](backend/.goreleaser.yaml) and ISSUES.md.
...
- Signing: macOS ships as an arm64-only, Developer ID-signed, notarized, and stapled `Logsonic.app` zip. Windows installer/Authenticode is not implemented yet.
...
GoReleaser build post-hooks sign the arm64 Mach-O binary in place using the login-Keychain Developer ID identity. The codesign call is wrapped in [`backend/scripts/sign-macos.sh`](backend/scripts/sign-macos.sh).
